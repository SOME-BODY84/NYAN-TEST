...
BOOST_FIXTURE_TEST_SUITE(fastcoin_tests, TestingSetup)
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    for (nHeight = 0; nHeight <= 100000; nHeight++) {
        ...
        CAmount nSubsidy = GetFastcoinBlockSubsidy(nHeight, params, prevHash);
    ...
    const Consensus::Params& params = mainParams.GetConsensus(nHeight);
    CAmount nConstantSubsidy = GetFastcoinBlockSubsidy(nHeight, params, prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetFastcoinBlockSubsidy(700000, params, prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
